...
public class blueShellHandlerScript : MonoBehaviour {
	// This script can be used with any object that is supposed to follow a
	// route marked out by waypoints. adapted from initial framework for the blue shell


	public float vitesseAngulaire = 20.0f;
	public float vitesseDeProjection = 50.0f;
	public float tempsDeVieApresCibleAcquise = 2.0f;
	...
	void Start ()
	{
		...
		manager = GameObject.Find("Game Manager").GetComponent<CheckpointManager>();
	...
	{
		Transform firstPlace = manager.findFirstPlace ();
	...
	void Update()
	{
		...
		else
		{
			Transform firstPlace = manager.findFirstPlace();
			this.rigidbody.velocity = this.transform.forward * vitesseDeProjection;
			this.rigidbody.MoveRotation(Quaternion.RotateTowards(this.transform.rotation,Quaternion.LookRotation(firstPlace.position-this.transform.position),vitesseAngulaire));
	...
	void OnCollisionEnter (Collision hit)
	{
		if ((hit.transform.tag.Equals ("Player"))) 
		{
			...
			hit.transform.SendMessage("objectTouchedBlueShell",SendMessageOptions.DontRequireReceiver);
		}
	}
