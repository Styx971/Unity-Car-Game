...
public class CarController : MonoBehaviour
{
	...
	private float timerSpeedPowerUp = 1.0f;											// time for the power Speed Up
	private float timerScalePowerUp = 5.0f;											// time for the power of super sclae
	private bool isScalePowerUp = false;											// boolean to check if the car is in super scale mode or not 
	private Vector3 originalScaleCar;												// the originle scale of the car. 
	private bool isSpeedPowerUp = false;											// boolean to check if the car is in the boost accelerator
	private Dictionary<GameObject,float> dicForPickUp = new Dictionary<GameObject,float>();
	private float timerDisplayPickup= 5.0f;
	...
	public float bonusAccelerateurBoost =  15.0f;
	...
	{
		get { return isScalePowerUp; }
	}
	public bool isSpeedUp
	{
		get { return isSpeedPowerUp; }
	}
	...
	void Start () {
		originalScaleCar = transform.localScale; // save the initial scale
		PickUps = GameObject.FindGameObjectsWithTag("PickUp");
		
		foreach (GameObject _obj in PickUps) {
			dicForPickUp.Add(_obj,0.0f);
		}
	...
	{
		if(isScalePowerUp){
			timerScalePowerUp -= Time.deltaTime;
			if(timerScalePowerUp < 0)
			{
				isScalePowerUp = false;
		...
		if(isSpeedPowerUp)
		{
			...
			timerSpeedPowerUp -= Time.deltaTime;
			if(timerSpeedPowerUp < 0)
			{
				isSpeedPowerUp = false;
				foreach (var wheel in wheels) {
					wheel.ISWHEELSPEEDUP=false;
				}
				timerSpeedPowerUp= 1.0f;
			}
		}
	...
	void OnTriggerEnter(Collider other){
		...
		if (other.gameObject.tag == "BonusAccelerator"){
			isSpeedPowerUp = true ;
			rigidbody.AddForce(bonusAccelerateurBoost*this.transform.forward, ForceMode.Impulse);
			foreach (var wheel in wheels) {
				wheel.ISWHEELSPEEDUP=true;
			}
		}
	}
	void setScale()
	{
		...
		{
			rigidbody.centerOfMass/=2;
			transform.localScale= originalScaleCar;
			timerScalePowerUp=5.0f;
		}
	}
