...
public class Wheel : MonoBehaviour
{
	...
	public Transform BoostSliderPrefab;
    public static Transform skidTrailsDetachedParent;
	public static Transform BoostSliderParent;
	...
	private bool isSpeedUpWheel= false;
	...
	private Transform SimpleFlame;
    private bool leavingSkidTrail;
	private bool leavingFlameTrail;
    ...
    void Start()
    {
		...
		if(BoostSliderParent==null)
		{
			BoostSliderParent = new GameObject("Boost Slider - Detached").transform;
		}

    }
	public bool ISWHEELSPEEDUP
	{
		get { return isSpeedUpWheel; }
		set{ isSpeedUpWheel = value; }
	}
	void Update(){
    ...
    void FixedUpdate()
    {
		...
		if (BoostSliderPrefab != null) {
			if(isSpeedUpWheel && OnGround )
			{
				if(!leavingFlameTrail){
					SimpleFlame = Instantiate (BoostSliderPrefab) as Transform;
					if (SimpleFlame != null) {
						SimpleFlame.parent = transform;
						SimpleFlame.transform.position = this.transform.position;
						Quaternion localTemp = new Quaternion(this.transform.rotation.x,-180+this.transform.rotation.y,this.transform.rotation.z,this.transform.rotation.w);
						SimpleFlame.transform.localRotation =localTemp;
					}
					leavingFlameTrail= true;
				}
			} else {
				if (leavingFlameTrail) {
					SimpleFlame.parent= BoostSliderParent;
					Destroy (SimpleFlame.gameObject);
					leavingFlameTrail = false;
				}
			}
		}
